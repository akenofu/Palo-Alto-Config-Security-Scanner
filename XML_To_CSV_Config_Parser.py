		...
		try:
			for t in entry.find(i).findall('member'):
				...
				if i == 'source' or i == 'destination':
					try:
						alias_decoded.append("%s"%aliases[t.text]) 
					except KeyError:
					...
					if t.text == 'application-default':
						for protocol in out[-1].split('\n'):
							try:
								protos.append("%s"%ports[protocol])
							except KeyError:
								try: 
									protos.append("%s (%s)"%(str(getservbyname(protocol)),protocol))
								except OSError:
									protos.append("Undefined (%s)"%protocol)
					else:
						protos.append("%s"%ports[t.text])
			...
			if alias_decoded != []:
				...
				out.append(''.join(alias_decoded))
			elif protos != []:
				out.append(''.join(protos))
